'use strict';
  ...
  addToCompairButtonsDOM.forEach(addToCompairButtonDOM => {
    addToCompairButtonDOM.addEventListener('click', () => {
      ...
      //   console.log(compairItem.name === compairOBJ.name);
      // });

      const inInCompair = (compairList.filter(compairItem => (compairItem.name === compairOBJ.name)).length > 0 );
      if(inInCompair === false){
