(function() {
  ...
  const addToCompairButtonsDOM = document.querySelectorAll(
    '[data-action="ADD_TO_COMPAIR"]'
  );
  ...
  if (compairList.length > 0) {
    compairList.forEach(compairItem => {
      ...
      compairListDOM.insertAdjacentHTML(
        "beforeend",
        `
              <div class="compair__item">
                <img class="compair__item__image" src="${product.image}" alt="${
          product.name
        }" />
        ...
        if (
          ...
          addToCompairButtonsDOM.forEach(addToCompairButtonDOM => {
            addToCompairButtonDOM.addEventListener("click", () => {
              ...
              const isInCompair =
                compairList.filter(
                  compairItem => compairItem.name === compairOBJ.name
                ).length > 0;
              ...
              if (!isInCompair) {
                compairListDOM.insertAdjacentHTML(
                  ...
                  `
                        <div class="compair__item">
                          <img class="compair__item__image" src="${
                            compairOBJ.image
                          }" alt="${compairOBJ.name}" />
                          <h3 class="compair__item__name">${
                            compairOBJ.name
                          }</h3>
                          <h3 class="compair__item__price">${
                            compairOBJ.price
                          }</h3>
                          <button class="btn btn--danger btn-small" data-action="REMOVE_ITEM">&times;</button>
                        </div>
                ...
                localStorage.setItem(
                  "compairList",
                  JSON.stringify(compairList)
                );

                addToCompairButtonDOM.innerText = "In Compair List";
                addToCompairButtonDOM.disabled = true;

                const compairItemsDOM = document.querySelectorAll(
                  ".compair__item"
                );

                compairItemsDOM.forEach(compairItemDOM => {
                  if (
                    compairItemDOM.querySelector(".compair__item__name")
                      .innerText === compairOBJ.name
                  ) {
                    compairItemDOM
                      ...
                      .addEventListener("click", () => {
                        compairList.forEach(compairItem => {
                          if (compairItem.name === compairOBJ.name) {
                            compairItemDOM.classList.add(
                              "compair__item--removed"
                            );
                            setTimeout(() => compairItemDOM.remove(), 250);
                            compairList = compairList.filter(
                              compairItem =>
                                compairItem.name !== compairOBJ.name
                            );
